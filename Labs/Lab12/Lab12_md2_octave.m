...
% -> suitable for GNU Octave (keystroke input converted to line + [ENTER] input)
%
clear;
clf;
%
...
n = 0;
i = 0;
while i < Ngrid       % arrange particles on a roughly square lattice
   j = 0;             % to keep them apart initially
   while j < Ngrid
      n = n + 1;
      if n <= Npart
         x(2,n) = (i + 0.5) * Lgrid + dmax * (rand - 0.5) * Lgrid * sqrt(2);
         y(2,n) = (j + 0.5) * Lgrid + dmax * (rand - 0.5) * Lgrid * sqrt(2);
...
%
%symSize = 20. * (20. / L)**2;  %symbol size (area)
symSize = 20. * (20. / L);
